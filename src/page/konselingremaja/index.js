...
import { KonselingRemajaURL, MYAPP, getData, storeData } from '../../utils/storedata'

const KonselingRemaja = ({navigation}) => {
   const kembali = () => {
        navigation.goBack();
    }
...
if ((form.nik.length == 0) || (form.nama_sekolah.length == 0) || (form.nama_desa.length == 0)) {
  ...
  .then(response => {
    ...
    if (response.data==200) {
      ...
      navigation.replace('HalamanKonseling');
    } else {
...
<ScrollView>
  <View style={{padding:10, alignItems:'center', marginTop:10}}>
    ...
    <Text style={{fontFamily:'Poppins-Regular', fontSize:12, marginTop:30, textAlign:'center'}}>Silahkan Daftar terlebih dahulu agar bisa konseling</Text>
  </View>
...
<Text style={{color:'white', fontFamily:'Poppins-SemiBold', textAlign:'center'}}>Daftar</Text>
</TouchableOpacity>
{/* END KIRIM */}
{/* KIRIM */}
<TouchableOpacity onPress={() => navigation.navigate('HalamanKonseling')} style={{padding:10, marginTop:20,}}>
<Text style={{color:'black', fontFamily:'Poppins-SemiBold', textAlign:'center'}}>Saya sudah daftar</Text>
</TouchableOpacity>
{/* END KIRIM */}
</View>
...
export default KonselingRemaja
